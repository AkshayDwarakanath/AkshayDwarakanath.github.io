---
title: "Part 2 - OAuth 2.0, OIDC, SSO & Access Tokens — A Practical Primer for Modern Web Apps"
...
It sets the stage for a **hands-on interactive demo** (Next.js + Node.js/Python) that I’ll cover in later posts.
...
> - You sign in with Google → allow access → the third party app receives a **access token** that grants only the permission you approved to access your profile info.
...
sequenceDiagram
   ...
   Note over User, API: 1. OIDC Authentication Step (Who are you?)
   User->>Browser: Clicks "Sign in with Google"
   Browser->>WebApp: Request Login
   WebApp-->>Browser: Redirect to Google (IdP) with Scope="openid profile"
   Browser->>IdP: GET /authorize (Login Page)
   ...
   Note right of WebApp: ID Token verifies User identity (OIDC)


   Note over User, API: 2. OAuth Authorization Step (What can you do?)
   WebApp->>API: Request Data (files) using Access Token. (Authorization: Bearer Access Token)
   API->>IdP: (Optional) Validate Access Token
   IdP-->>API: Token Valid
   API-->>WebApp: Return Requested Data (files)
   WebApp-->>Browser: Display User Profile & Data (files)
```
...
sequenceDiagram
   ...
   Note over User,Jira: SSO via OIDC/SAML — one login at IdP, sessions for multiple apps
   ...
   Note over Browser,IdP: IdP session active → no prompt for next apps
